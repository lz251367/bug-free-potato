(function (global, factory) {
  ...
  function generateIntent(config, options) {
    ...
    var intentParam = keys
      .map(function (key) {
        return "".concat(key, "=").concat(intent[key], ";");
      })
      .join("");
    var intentTail = "#Intent;"
      .concat(intentParam, "S.browser_fallback_url=")
      .concat(encodeURIComponent(fallback), ";end;");
    ...
    if (typeof outChain !== "undefined" && outChain) {
      ...
      return "intent://"
        .concat(path, "?")
        .concat(key, "=")
        .concat(encodeURIComponent(urlPath))
        .concat(intentTail);
    }
  ...
  function generateYingYongBao(config, options) {
    ...
    return ""
      .concat(options.yingyongbao, "&android_schema=")
      ...
      },
      {
        ...
        value: function open(config) {
          ...
          if (isIos) {
            ...
            if (
              (isWechat && semverCompare(getWeChatVersion(), "7.0.5") === -1) ||
              isWeibo
            ) {
